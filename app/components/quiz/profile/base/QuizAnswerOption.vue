...
<template>
  ...
  >
    ...
    <span
      v-if="showCorrect"
      class="flex items-center justify-end gap-1 text-xs font-bold text-green-800 dark:text-green-500"
    >
    ...
    <span
      v-if="showWrongSelected"
      class="flex items-center justify-end gap-1 text-xs font-bold text-red-500 dark:text-red-300"
    >
      {{ $t("quiz.your_answer") }}
