...
<template>
  <CardComponent>
    <NuxtLink class="block" :to="`/quizzes/${quiz.id}`">
      <div class="flex justify-between items-start p-4 pb-0">
        ...
        <!-- Level Badge -->
        <BadgeComponent :text="quiz.levelLabel[locale]" :type="quiz.level"/>
      </div>
      <div class="p-4">
        <!-- Category -->
        <p class="text-sm opacity-70 mb-1">{{ quiz.category.name }}</p>
        <!-- Title -->
        <h3 class="text-sm font-bold line-clamp-1">{{ quiz.title[locale] }}</h3>
        <!-- Description -->
        <p class="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mt-3" :title="quiz.description[locale]">
